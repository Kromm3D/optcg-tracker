...
Los datos se obtienen scrapeando el **sitio oficial** de One Piece Card Game
([en.onepiece-cardgame.com/cardlist](https://en.onepiece-cardgame.com/cardlist/)),
recorriendo todas las series (boosters OP, EB, PRB, starter decks ST, promos…).
...
# 2. Genera solo el índice (rápido: metadatos de todas las series)
python scripts/build_card_database.py --index-only

# 3. Genera el índice + descarga las imágenes que falten (sets nuevos)
python scripts/build_card_database.py
```

El script incluye una pausa entre peticiones para **no sobrecargar el sitio
oficial**. Por favor, no la reduzcas en exceso.
