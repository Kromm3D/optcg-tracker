// PublicWishlistScreen — la wishlist de alguien, vista desde un enlace
// compartido. Mismo patrón de sólo lectura que PublicBinderScreen (sin
// asumir sesión, con CTA final) pero para wishlist_cards en vez de
// collection_items — ver lib/publicWishlist.ts.

import React, { useEffect, useMemo, useState } from 'react';
import { ActivityIndicator, Pressable, ScrollView, StyleSheet, Text, View } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import type { PublicWishlistScreenProps } from '../navigation';
...
import { fetchPublicWishlist, type PublicWishlist } from '../lib/publicWishlist';
import type { WishlistCard } from '../types';

export function PublicWishlistScreen({ route, navigation }: PublicWishlistScreenProps) {
  const { username } = route.params;
  const t = useT();
  const insets = useSafeAreaInsets();
  // undefined = cargando, null = nada público que enseñar.
  const [pw, setPw] = useState<PublicWishlist | null | undefined>(undefined);

  useEffect(() => {
    void fetchPublicWishlist(username).then(setPw);
  }, [username]);

  const entries = useMemo(() => {
    if (!pw) return [];
    const all: WishlistCard[] = pw.wishlists.flatMap((wl) => Object.values(wl.cards));
    return all.sort((a, b) => `${a.code}${a.suffix}`.localeCompare(`${b.code}${b.suffix}`, undefined, { numeric: true }));
  }, [pw]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <View style={[s.header, { paddingTop: insets.top + 12 }]}>
        ...
        {pw ? (
          <Text style={s.headerSub}>{t('public.wishCount', { n: entries.length })}</Text>
        ) : null}
      </View>
